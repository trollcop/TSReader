...
static int cmd_streaminfo_internal(const char *prefix_str)
{
	/* Start. */
	int ret = hdhomerun_device_detect_programs_start(hd);
	if (ret < 0) {
		fprintf(stderr, "communication error sending request to hdhomerun device\n");
		return -1;
	}
	if (ret == 0) {
		fprintf(stderr, "device does not support operation\n");
		return 0;
	}

	/* Process. */
	uint64_t stop_time = getcurrenttime() + 5000;
	while (getcurrenttime() < stop_time) {
		usleep(64000);
		hdhomerun_device_detect_programs_recv(hd);
	}

	/* Stop. */
	hdhomerun_device_detect_programs_stop(hd);

	/* Display result. */
	struct hdhomerun_ts_program_entry_t *entry = hdhomerun_device_detect_programs_result(hd);
	while (entry) {
		printf("program %s%u %u.%u",
			prefix_str,
			(unsigned int)entry->program_number,
		...
		char *filter = hdhomerun_device_program_detect_filter(hd, entry->program_number);
		if (filter) {
...
static int cmd_streaminfo(const char *tuner_str)
{
	...
	return cmd_streaminfo_internal("");
}
...
static int cmd_scan(const char *tuner_str, const char *start_value)
{
	...
	while (1) {
		...
		/* Detect sub channels. */
		char prefix_str[64];
		sprintf(prefix_str, "%s-", channel_str);
		cmd_streaminfo_internal(prefix_str);
...
static int main_internal(int argc, char *argv[])
{
	...
	if (hdhomerun_device_get_version(hd, NULL) < 0) {
