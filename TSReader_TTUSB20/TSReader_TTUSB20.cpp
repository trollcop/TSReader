...
int nDiSEqCInput;
#ifndef DVBS2
char gszSourceName[] = {"TechnoTrend USB2.0 DVB-S"};
#else DVBS2
char gszSourceName[] = {"TechnoTrend USB2.0 DVB-S2"};
...
char gszSourceName[] = {"TechnoTrend USB2.0 DVB-T"};
#endif DVBT
...
int fSpectrumInversion;
int nBandwidth;
char gszSourceName[] = {"TechnoTrend USB2.0 DVB-C"};
#endif DVBC
...
DWORD WINAPI ReadTTThread(LPVOID lpv)
{
	...
	int retlen;
	int nPollCounter = 100;
	__int64 nLoopCount = 0;
	BYTE bBuffer[RX_SIZE];
	//HANDLE hDebugFile = CreateFile("c:\\tsreader.ts", GENERIC_WRITE, 0, (LPSECURITY_ATTRIBUTES)NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, (HANDLE)NULL);
	...
	while (!ss->fTerminateReadThread)
	{
		...
		retlen = m_BoardControl.TSRecordRead(bBuffer, RX_SIZE);
		if (retlen > 0)
		{
			if (nLoopCount++ > 10)
			{				
				SourceHelper_SyncData(bBuffer, retlen);
				//{
				//	DWORD dwWritten;
				//	WriteFile(hDebugFile, bBuffer, retlen, &dwWritten, NULL);
				//}
		...
		//else
		{
			if (nPollCounter++ > 100)
			{
				...
				{
					EnterCriticalSection(&csSignal);
					lstrcpy(szLastSignalReport, "m_FE.GetState() failed");
					LeaveCriticalSection(&csSignal);
				}
				nPollCounter = 0;
			}
...
BOOL TSReader_Start()
{
	...
	m_BoardControl.EnableIsoStream(CDVBBoardControl::BANDWIDTH_60);
...
BOOL TSReader_Stop()
{
	...
	m_BoardControl.EnableIsoStream(CDVBBoardControl::BANDWIDTH_NONE);
...
BOOL TSReader_Init(PSOURCESTRUCT pss)
{
	...
	if (fFirstTime == TRUE)
	{
		...
		for (nDevice = 0; nDevice < nDevices; nDevice++)
		{
			...
			if(CDVBCommon::OpenDevice(nDevice, "TSReader", TRUE))
			{
				DVB_ERROR err = m_FE.Init();
#ifdef DVBS
#ifndef DVBS2
		...
		if (nDevice == nDevices)
		{
			//MessageBox(ss->hWndTSReader, "Unable to locate a suitable USB2.0 device", gszSourceName, MB_ICONSTOP);
			return FALSE;
		}
#ifdef DVBT
	}
...
/*
			...
			nInput = ss->nDiSEqCInput - 1;
			if ((nInput >= 0) && (nInput <= 3) )
			{
				BYTE bDiSEqCCommand[4] = {0xe0, 0x10, 0x38, 0x00};

				bDiSEqCCommand[3] = bPositionByte[nInput];
				m_FE.SendDiSEqCMsg(bDiSEqCCommand, 4, FALSE);
			}
*/
		}
		break;
