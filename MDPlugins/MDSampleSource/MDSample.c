...
char szThisFilterName[] = {"DishNetwork Explorer"};
...
BOOL ParsePATPacket(BYTE * pSectionPointer, int nPacketLength)
{
	...
	nTransportStreamID = (pSectionPointer[3] << 8) + pSectionPointer[4];		// Network ID
	nVersionNumber = (pSectionPointer[5] >> 1) & 0x1f;
	nSectionNumber = pSectionPointer[6];
	nLastSectionNumber = pSectionPointer[7];

	nNetworkDescriptorLength = ((pSectionPointer[8] << 8) + pSectionPointer[9]) & 0xfff;

	//for (i = 0; i < nNetworkDescriptorLength; i++)
	//{
	wsprintf(szDecodedPAT, "Length: %i, First Character: %c", nNetworkDescriptorLength, pSectionPointer[10]);
		...
		if ((pBuffer[nBufferOffset + 1] & 0x40) == 0x40)		// PES/PSI start?
		{
			...
			if (nFillPtr > 0)
			{
				...
				if (ParsePATPacket(bSIBuffer, nFillPtr) == TRUE)
				{
					...
					SendMessage(hTSReaderWnd, WM_COMMAND, ID_DECODENIT, 0);
...
void On_Menu_Select(unsigned int nMenuID)
{
	...
	case ID_DECODENIT:
		if (fRunning == FALSE)
		{
			...
			Filter.Pid = 0x0010;		// PID containing the NIT
