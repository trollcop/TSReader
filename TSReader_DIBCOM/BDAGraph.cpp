					 ...
					 LPVOID callback_data)
{
	...
	{
		ErrorBox("Error connecting infinite tee to tuner device");
		Destroy();
		return 1;
	}
...
int BDAGraph::LoadTuningSpace()
{
	...
	/* create instance of tuning space container */

	if (FAILED((hr=pTuningSpaceContainer.CoCreateInstance(CLSID_SystemTuningSpaces))))
	{
		ErrorBox("Cannot create tuning space container");
		return 1;
	}

	/* retrieve the tuning space we are after by ID */

	if (FAILED((hr=pTuningSpaceContainer->get_Item(CComVariant(networkType), &pTuningSpace))))
	{
		ErrorBox("Cannot find tuning space");
		return 1;
	}
...
int BDAGraph::LoadNetworkProvider()
{
	...
	CComBSTR providerCLSIDstr;
	if (FAILED((hr=pTuningSpace->get_NetworkType(&providerCLSIDstr))))
	{
	...
	if (FAILED((hr=CLSIDFromString(providerCLSIDstr, &providerCLSID))))
	{
		ErrorBox("Error converting CLSID into GUID form");
		return 1;
	}

	if (FAILED((hr=CoCreateInstance(providerCLSID,
									NULL,
									CLSCTX_INPROC_SERVER,
									IID_IBaseFilter,
									reinterpret_cast<void**> (&pNetworkProvider)))))
	{
