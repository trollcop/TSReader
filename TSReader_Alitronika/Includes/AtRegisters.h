...
#ifndef TASKING_FIRMWARE
class CATBoard;
class CAtRegisters;
#endif // TASKING_FIRMWARE
...
/** \brief The register definition structure.
 *
 *	The (accessible) registers of an AT Device.\n
 *	The Register bits control the AT-device electronics directly. To enable a
 *	feature, activate the corresponding bit in the register set.
 *	This method of control is efficient, and once you know the register bits,
 *	it is not difficult to realize.
 *	\see CAtRegisters
 */
struct ATDV_API_API ATREGISTRY
{
...
typedef struct ATREGISTRY ATFPGAREGS;

/*@} public_api_regs */


#define ATREGS_DVB_MAGIC_WORD	((s16) -1123)	///< eeprom magic word

typedef struct _EMB_ATFPGAREGS
{
	s16			m_nMagicWord;		///< magic WORD to see if EEPROM is blank
	ATFPGAREGS	m_AtFpgaRegs;
} EMB_ATFPGAREGS;
...
/** \brief The access to the AT-device's FPGA registers.
 *
