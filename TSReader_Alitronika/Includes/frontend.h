...
/**	
 *	The modulation mode list is a single combined list for all methods.\n
 *	For different receivers, only a sub-set of the given types may be used.
 */
...
/**	
 *	The guard interval is used to prevent false symbol detection.
 *	It is only used in a few new (like COFDM) modulation methods.
 */
...
typedef enum src_dvbc_mode
{
	DVB_C_MODE_ANNEXA,	///< annex a mode
	DVB_C_MODE_ANNEXB,	///< annex b mode
	DVB_C_MODE_ANNEXC,	///< annex c mode
	DVB_C_MODE_DVBC,	///< dvb c mode
	DVB_C_MODE_FREE		///< free mode
} src_dvbc_mode_t;
...
struct dvb_at2700_parameters {
		u8							annexb_enable;			///< 1: enable annexB, 0: disable annexB (annexA/C mode enabled)
        u32							symbol_rate;			///< symbol rate in Symbols per second
		...
		src_dvbc_mode_t				dvbc_mode;				///< store selected dvbc_mode
};
...
struct dvb_at2800_parameters {
		...
		u8							dvb_h_en;				///< enable dvb-h mode in ofdm fpga core
		u16							dvb_h_cell_id;			///< cell id number for dvb-h
		u8							dvb_h_indepth_int;		///< enable indepth interleaver for dvb-h
		u8							dvb_h_HP_time_slice;	///< enable high priority time slicing for dvb-h
		u8							dvb_h_HP_MPE_FEC;		///< enable high priority MPE FEC for dvb-h
		u8							dvb_h_LP_time_slice;	///< enable low priority time slicing for dvb-h
		u8							dvb_h_LP_MPE_FEC;		///< enable low priority MPE FEC for dvb-h
};

/** Modulator RF anf IF parameters
 *  Used for AT2700, and AT2800 modulators
 */
struct dvb_if_rf_parameters {
	u8	RfLocked;		///< status: 1: RF pll has locked, 0: RF pll is not locked
	u8	RfIfSel;		///< selects if RF output is selected or IF output. 1: RF is selected, 0: IF is selected
...
{
	u8		m_IsValid;	///< Must be non-zero to carry a valid SNR and EVM
...
*	\see	\ref ATBoardTunDVBT (DVB-T)
*	\see	\ref ATBoardTunDVBCAnnexA (DVB-C)
*	\see	\ref ATBoardTunDVBCAnnexB (DVB-C)
*	\see	\ref ATBoardModDVBT (DVB-T/H Modulator)
*	\see	\ref ATBoardModDVBC (DVB-C Modulator)
*/
/*@{*/
...
/**  
 *	This ioctl call returns the bit error rate \b after the FEC (forward error correction) for the
...
/**  
 *	This ioctl call returns the bit error rate \b before the FEC (forward error correction) for the
 *	signal currently received/demodulated by the dvb source.
 *	Application: AT72x.
 *	@param arg uint32_t *.
 *
...
/** 	
*	This ioctl call stores the tuner parameters in EEPROM.
*/
#define SRC_STORE_TUN_PARAMS			0x0023

/** 	
*	This ioctl call restores the tuner parameters in EEPROM.
*/
#define SRC_RESTORE_TUN_PARAMS			0x0024

/** 	
*	This ioctl call stores the modulators RFIF parameters in EEPROM.
*	\internal This function must not be used by application programmers. It
*	will damage calibration settings of boards.
*/
#define SRC_STORE_MOD_RFIF				0x0025

/** 	
*	This ioctl call restores the modulators RFIF parameters in EEPROM.
*	\internal This function must not be used by application programmers. It
*	will damage calibration settings of boards.
*/
#define SRC_RESTORE_MOD_RFIF			0x0026

/** 	
*	This ioctl call stores the modulators DVB parameters in EEPROM.
*	\internal This function must not be used by application programmers. It
*	will damage calibration settings of boards.
*/
#define SRC_STORE_MOD_DVB				0x0027

/** 	
*	This ioctl call restores the modulators DVB parameters in EEPROM.
*	\internal This function must not be used by application programmers. It
*	will damage calibration settings of boards.
*/
#define SRC_RESTORE_MOD_DVB				0x0028
