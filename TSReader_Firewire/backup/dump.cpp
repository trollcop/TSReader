...
/*    
    if ((m_pDump->m_fsize == 0) && m_pDump->m_pNewFileName)
    {
        m_hTmpFile = m_pDump->m_hFile;
        m_pTmpFileName = m_pDump->m_pFileName;
        m_pDump->m_hFile = m_pDump->m_hNewFile;
        m_pDump->m_pFileName = m_pDump->m_pNewFileName;
        m_pDump->m_hNewFile = m_hTmpFile;
        m_pDump->m_pNewFileName = m_pTmpFileName;
    }
    m_pDump->m_fsize += sample_len;
  */  
...
STDMETHODIMP CDump::SetFileName(LPCOLESTR filename,const AM_MEDIA_TYPE *pmt)
{
    ...
    char extension[40];
    char *ext_p;
    LPCOLESTR pszFileName = (LPCOLESTR)tmpstr;
    
    
    if (!filename && !m_pFileName)
        return ERROR_FILENAME_EXCED_RANGE;
    
    if (!filename)
    {
        ext_p = strrchr(filestr,'_');
        wsprintf(ext_p,"_%03d",m_fcount++);
        ext_p[4] = '.';
    } else 
    {
        strcpy(filestr,(char *)filename);
        ext_p = strrchr(filestr,'.');
        if (ext_p)
        {
            strcpy(extension,ext_p);
            wsprintf(ext_p,"_%03d",m_fcount++);
            strcat(filestr,extension);
        } else
        {
            return ERROR_FILENAME_EXCED_RANGE;
        }        
    ...
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, filestr, strlen(filestr),
                        (LPWSTR)pszFileName, 1000);
...
/*
STDMETHODIMP CDump::SetNewFileName(LPCOLESTR filename,const AM_MEDIA_TYPE *pmt)
{
    // Is this a valid filename supplied
    char tmpstr[2000];
    char extension[40];
    ...
    {
        CloseNewFile();
        delete m_pNewFileName;
        m_pNewFileName = NULL;
    }
    
    m_pNewFileName = new WCHAR[1+lstrlenW(pszFileName)];
    if (m_pNewFileName == 0)
        return E_OUTOFMEMORY;

    lstrcpyW(m_pNewFileName,pszFileName);
...
HRESULT CDump::OpenNewFile()
{
    ...
    if(!WideCharToMultiByte(CP_ACP,0,m_pNewFileName,-1,convert,MAX_PATH,0,0))
        return ERROR_INVALID_NAME;

    pFileName = convert;
#else
    pFileName = m_pNewFileName;
#endif

    // Try to open the file

    m_hNewFile = CreateFile((LPCTSTR) pFileName,   // The filename
                         GENERIC_WRITE,         // File access
                         FILE_SHARE_READ,       // Share access
                         NULL,                  // Security
                         CREATE_ALWAYS,         // Open flags
...
HRESULT CDump::CloseNewFile()
{
    ...
    m_hNewFile = INVALID_HANDLE_VALUE; // Invalidate the file 
