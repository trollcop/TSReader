...
//#include <dshowutil.h> // for GetOutPin, GetInPin
#include <xprtdefs.h>
...
HRESULT findTuner(IBaseFilter	**tunerFilter, char *filtername, REFCLSID inCLSID)
{
	...
	{
		char szTemp[256];
		wsprintf(szTemp, "Firewire: Looking for device '%s'\n", filtername);
...
//        if(!lstrcmp(TEXT("Microsoft AV/C Tape Subunit Device"), m_DeviceName))
		{
			char szTemp[256];
			wsprintf(szTemp, "Firewire: Found device '%s'\n", m_DeviceName);
			OutputDebugString(szTemp);
		}
        if(!lstrcmp(TEXT(filtername), m_DeviceName))
        {
			{
				char szTemp[256];
				wsprintf(szTemp, "Firewire: Attempting to BindToObject '%s'\n", m_DeviceName);
				OutputDebugString(szTemp);
			}
			//printf("Found: %s\n", m_DeviceName) ;        
...
HRESULT SendCmdToDevice(IAMExtTransport *pTransport, int channel)
{
    ...
    long cbCmd = 8;
//    hr = pTransport->GetTransportBasicParameters(ED_TRANSBASIC_OUTPUT_SIGNAL, 
//                                                 &cbCmd,
//                                                 (LPOLESTR*) &AvcCmd);
...
//    hr = pSink->SetFileName((LPCOLESTR)filename, NULL);
    ...
    {
        cDump->Release();
		OutputDebugString("Firewire: pSink->SetFileName() failed\n");
        return -1;
    }    
    ...
    //hr = findTuner(&pTuner, "Microsoft AV/C Tape Subunit Device", CLSID_VideoInputDeviceCategory);
    hr = findTuner(&pSource, "Panasonic MPEG2TS Tape Subunit Device", CLSID_VideoInputDeviceCategory);    
    if (hr != S_OK)
    {
	    hr = findTuner(&pTuner, "Microsoft AV/C Tape Subunit Device", CLSID_VideoInputDeviceCategory);
	    if (hr != S_OK)
		{
		    hr = findTuner(&pTuner, "Motorola AV/C Tuner Device", CLSID_VideoInputDeviceCategory);			
			if (hr != S_OK)
			{
				OutputDebugString("Firewire: Could not find Firewire Capture Device\n");
				cDump->Release();
				return 0;
			}
		}
