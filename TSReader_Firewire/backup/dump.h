...
class CDump : public CUnknown, public IFileSinkFilter
{
    ...
    HANDLE   m_hNewFile;               // Handle to file for dumping
    LPOLESTR m_pNewFileName;           // The filename where we dump
    BOOL     m_fWriteError;
    int      m_fcount;              // file counter
    UINT64   m_fsize;              // file counter
    char filestr[2000];
	int nTSBufferIndex;
public:
    ...
    STDMETHODIMP SetFileName(LPCOLESTR filename,const AM_MEDIA_TYPE *pmt);
    STDMETHODIMP SetNewFileName(LPCOLESTR filename,const AM_MEDIA_TYPE *pmt);
