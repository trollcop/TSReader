...
public:
	...
	void SetInitialTune(LONG, LONG, LONG, LONG, LONG, LONG);
	int Destroy();
	int Play();
	int Stop();
	int GetTuneStatus(LONG*, BOOLEAN*,BOOLEAN*,LONG*,LONG*);
	int GetDeviceName(LPWSTR*);
	int TuneDVB(LONG, LONG, LONG, LONG, LONG, LONG);
	int IsCreated()
	{
...
protected:
	...
	int CreateDVBCTuneRequest(LONG, LONG, LONG, LONG, LONG, LONG); 
	...
	LONG initial_symbolrate;
	LONG initial_modulation;
	LONG initial_ONID;
	LONG initial_SID;
	LONG initial_TSID;
