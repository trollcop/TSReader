...
int read_new_slice()
{
  ...
  long ftell_position = 0; // rh
  long expected_slice_type;
  int retval = FALSE; //rh
...
//rh  while (1)
  while (!retval) //rh
  {
//rh    ftell_position = ftell(bits);

//rh    if (input->FileFormat == PAR_OF_ANNEXB)
      ret=GetAnnexbNALU (nalu);
	  if (ret == -2)
		  return EOS;
//rh    else
//rh      ret=GetRTPNALU (nalu);
...
//      printf ("read_new_slice: returning %s\n", "EOS");
    ...
    switch (nalu->nal_unit_type)
    {
      ...
      case NALU_TYPE_IDR:
        ...
        if(is_new_picture())
        {
          retval = init_picture(img, input); //rh
 ...
 ************************************************************************
 */
int init_picture(struct img_par *img, struct inp_par *inp) //rh
{
  int i,k,l;
  Slice *currSlice = img->currentSlice;
  int retval = FALSE; //rh
...
int  exit_picture() //rh
{
  ...
  retval = store_picture_in_dpb(dec_picture); //rh
  dec_picture=NULL;
