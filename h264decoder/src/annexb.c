...
unsigned char rh_fgetc(FILE * f)
{
rh_fgetc_return_data:
	if (buffer_bytes)
	{
		buffer_bytes--;
		return buffer[++buffer_offset];
	}
	while (buffer_bytes < READ_BUFFER_SIZE)
	{
		DWORD dwRead;
		ReadFile(hInput, &buffer[buffer_bytes], READ_BUFFER_SIZE - buffer_bytes, &dwRead, NULL);		
		if (dwRead == 0)
		{
			OutputDebugString("TSReader: H264: EOF\n");
			EOFFlag = TRUE;
			return 0;
		}
		buffer_bytes += dwRead;
...
int GetAnnexbNALU (NALU_t *nalu)
{
  ...
  {
	   while(!rh_feof(bits) && (Buf[pos++]=rh_fgetc(bits))==0);
	   FirstTime = FALSE;
  }
