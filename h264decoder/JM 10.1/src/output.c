...
void write_out_picture(StorablePicture *p, int p_out)
{
  ...
  //rh buf = malloc (p->size_x*p->size_y*symbol_size_in_bytes);
  //rh if (NULL==buf)
  if (NULL==phd->pY) // rh
  {
  ...
  if(rgb_output)
  {
    ...
    //rh img2buf (p->imgUV[1], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
    //rh write(p_out, buf, (p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)*symbol_size_in_bytes);
...
//rh  img2buf (p->imgY, buf, p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
  img2buf (p->imgY, phd->pY, p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom); //rh
  phd->y = (p->size_y-crop_bottom-crop_top); //rh
  phd->x = p->size_x-crop_right-crop_left; //rh
//rh  write(p_out, buf, (p->size_y-crop_bottom-crop_top)*(p->size_x-crop_right-crop_left)*symbol_size_in_bytes);
...
//rh    img2buf (p->imgUV[0], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
//rh    write(p_out, buf, (p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)* symbol_size_in_bytes);
    img2buf (p->imgUV[0], phd->pU, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom); //rh

    if (!rgb_output)
    {
//rh      img2buf (p->imgUV[1], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
//rh      write(p_out, buf, (p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)*symbol_size_in_bytes);
      img2buf (p->imgUV[1], phd->pV, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom);
    }
  }
  else
  {
 ...
 ************************************************************************
 */
int write_stored_frame( FrameStore *fs,int p_out) //rh
{
	int retval = 0; //rh
