...
static int  output_one_frame_from_dpb(); //rh
 ...
 ************************************************************************
 */
int store_picture_in_dpb(StorablePicture* p) //rh
{
  int retval = FALSE; //rh
  unsigned i;
  int poc, pos;
...
//rh  while (dpb.used_size==dpb.size)
  while (dpb.used_size==dpb.size && !retval) //rh
  {
    ...
    // flush a frame
    retval = output_one_frame_from_dpb(); //rh
  }
...
//  printf ("output frame with frame_num #%d, poc %d (dpb. dpb.size=%d, dpb.used_size=%d)\n", dpb.fs[pos]->frame_num, dpb.fs[pos]->frame->poc, dpb.size, dpb.used_size);
  ...
  retval = write_stored_frame(dpb.fs[pos], p_out); //rh
