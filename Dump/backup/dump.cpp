...
//
CFactoryTemplate g_Templates[]= {L"TSReader_B2C2_Dump", &CLSID_Dump, CDump::CreateInstance, NULL, &sudDump
};
...
{
	int nOffset = 0;
	int nCompareSize = TS_BUFFER_SIZE;

	if (fIsDemuxFree == FALSE)
	{
		if (ss->nPATCATProcessed != 3)
			nCompareSize = TS_BUFFER_SIZE / 16;
	}
	else
	{
		nLoopCount++;
	...
	if (ss->tsb[nTSBufferIndex].nSize + lData >= nCompareSize)
	{
		//if (lnPacketCounter++ > 4)
		{
	...
	for (nOffset = 0; nOffset < lData; nOffset += 188)
	{
		int nPID = ( (*(pbData + nOffset + 1)) << 8 | (*(pbData + nOffset + 2)) ) & 0x1fff;
		if (fPIDActive[nPID] == FALSE)
			fPIDActive[nPID] = TRUE;
	}
...
STDMETHODIMP CDump::SetFileName(LPCOLESTR pszFileName,const AM_MEDIA_TYPE *pmt)
{
	...
	lnPacketCounter = 0;
	nLoopCount = 0;
	sscanf((char *)pszFileName, "%x %d", &nSS, &fIsDemuxFree);
	ss = (PSOURCESTRUCT)nSS;

	for (nPID = 0; nPID < 8192; nPID++)
		fPIDActive[nPID] = FALSE;
	wsprintf((char *)pszFileName, "%x", (int)&fPIDActive[0]);
	{
		char szTemp[128];
