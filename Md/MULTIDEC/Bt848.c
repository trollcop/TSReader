...
char Zeile[255];
	...
	if(!Alloc_DMA(98304,&Risc_dma,DMA_KERNEL_BUFFER_ALLOC))
	{
		...
		MessageBox(hWnd,"Risc Memory nicht (98 KB physikalisch Linear) nicht verfügbar","Multi-Decode",MB_ICONSTOP|MB_OK);
		return(FALSE);
	}
...
BOOL Alloc_Surface_DMA( void)
{
 ...
 else if ( hSAA7146 != NULL ) WD_DMALock(hSAA7146->hWD,Surface_dma);
...
if ( hBT8X8 != NULL ) WD_DMALock(hBT8X8->hWD,Overlay_dma);
 else if ( hSAA7146 != NULL ) WD_DMALock(hSAA7146->hWD,Overlay_dma);
...
if ( !( nFlags&(BT848_CAP_CTL_CAPTURE_EVEN|BT848_CAP_CTL_CAPTURE_ODD))) {
	...
	} else 	if (( Current_Mode == 1 ) && ( Has_DVB == TRUE )) {
...
int PCI_Scan( void )
{
    ...
    hWD = WD_Open();
    if ( hWD == NULL ) {
	strcpy(Zeile,"Treiber MDHALL kann nicht geladen werden"); 
    Write_Log(Zeile);
...
int BT8X8_Open (BT8X8_HANDLE *phBT8X8, DWORD dwVendorID, DWORD dwDeviceID,  DWORD options, BOOL Lock)
{
	...
	if ( LoadExternBTDriver == TRUE ) {   
    	sprintf(SystemDir,"BT8X8 Treibererweiterung MDBT soll geladen werden");
        Write_Log(SystemDir);
	...
	if ( NT == TRUE ) {
        ...
        Write_Log(SystemDir);
		 if ( ! LoadDeviceDriver( "MultiDec BT-Treiber", Path, &BT_Plugin_Handle,TRUE  ) )  {
    	        sprintf(SystemDir,"BT8X8 NT-Treibererweiterung kann nicht geladen werden");
                Write_Log(SystemDir);
		 };
		};
	} else {
    	...
    	sprintf(SystemDir,"BT8X8 Treibererweiterung MDBT soll nicht geladen werden");
        Write_Log(SystemDir);
	}
...
unsigned char MAC[8];
	...
	switch (message) {
		...
		if ( hBT8X8->Int.Int.fEnableOk == 1 ) {
					SetDlgItemText(hDlg,IDC_TEXT15,"Interrupt geladen");
			} else {
					SetDlgItemText(hDlg,IDC_TEXT15,"Interrupt nicht geladen");
			}

         if ( LoadExternBTDriver == FALSE ) {
        			SetDlgItemText(hDlg,IDC_TEXT37,"Treibererweiterung soll nicht geladen werden");
			} else {
				if ( ExternBTDriver == FALSE ) {
        			SetDlgItemText(hDlg,IDC_TEXT37,"Treibererweiterung konnte nicht geladen werden");
				} else {
				 ...
				 SetDlgItemText(hDlg,IDC_TEXT37,Text);
							...
							SetDlgItemText(hDlg,IDC_TEXT1,"Keine BT-Hardware gefunden");
			};
			...
			if ( hSAA7146->Int.Int.fEnableOk == 1 ) {
					SetDlgItemText(hDlg,IDC_TEXT32,"Interrupt geladen");
			} else {
					SetDlgItemText(hDlg,IDC_TEXT32,"Interrupt nicht geladen");
			}
			
            if ( ci_interface == 0 ) {
               SetDlgItemText(hDlg,IDC_TEXT34,"Kein CI-Interface gefunden");
			} else {
               SetDlgItemText(hDlg,IDC_TEXT34,"CI-Interface gefunden");
				   if ( ci_module[0].Present == 0 ) SetDlgItemText(hDlg,IDC_TEXT38,"Kein CICAM");
                     else  if ( ci_module[0].Present == 1 ) SetDlgItemText(hDlg,IDC_TEXT38,"CICAM gefunden");
                       else  if ( ci_module[0].Present == 2 ) SetDlgItemText(hDlg,IDC_TEXT38,"CICAM OK");
				   if ( ci_module[1].Present == 0 ) SetDlgItemText(hDlg,IDC_TEXT39,"Kein CICAM");
                     else  if ( ci_module[1].Present == 1 ) SetDlgItemText(hDlg,IDC_TEXT39,"CICAM gefunden");
			...
			if ( DVB_AUDIO == 0 ) SetDlgItemText(hDlg,IDC_TEXT35,"TI-DAC");
							...
							SetDlgItemText(hDlg,IDC_TEXT22,"Keine DVB-Hardware gefunden");
			};

            if ( lpSurface != NULL ) {
				if ( WTVBPP == 16 ) SetDlgItemText(hDlg,IDC_TEXT17,"Verfügbar RGB565(16-Bit)");
				  else if ( WTVBPP == 15 ) SetDlgItemText(hDlg,IDC_TEXT17,"Verfügbar RGB555(15-Bit)");
       				  else  SetDlgItemText(hDlg,IDC_TEXT17,"Verfügbar aber nicht 15-Bit/16-Bit ");
			} else SetDlgItemText(hDlg,IDC_TEXT17,"Nicht Verfügbar");
           ...
           if ( ddrval != DD_OK ) DriverCaps.dwCaps=0xffffffff;
            ...
            if ( Can_Overlay == TRUE ) {
			    SetDlgItemText(hDlg,IDC_TEXT18,"Verfügbar YUV2");
			    if ( Can_Overlay_Strech ) SetDlgItemText(hDlg,IDC_TEXT19,"Verfügbar");
				 else SetDlgItemText(hDlg,IDC_TEXT19,"Kein Overlay-Hardware-Streching");
		   } else {
			   SetDlgItemText(hDlg,IDC_TEXT18,"Nicht Verfügbar");
