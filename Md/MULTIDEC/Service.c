...
int Start_MDHal( void ) 
{
     ...
     sprintf(Path,"Betriebssystem : %s ",VersionString);
     Write_Log(Path);
	...
	if ( hWD == (void *)0xffffffff) {
	 ...
	 sprintf(Path,"Starte Service-Installation");
     Write_Log(Path);
		 ...
		 if(Device_Handle==INVALID_HANDLE_VALUE)
	{
		...
		if ( x != 50 ) return(-2);
    	 ...
    	 sprintf(Path,"Service bereits installiert");
         Write_Log(Path);
  		 WD_Close (hWD);
	};
...
// ( eigentlich schon immer ) Werbung für euch 
    ...
    if ( Register !=0xefcf1ffe ) {
    	 sprintf(Path,"Service - Register fehlerhaft ( Return : 0x%08x)",Register);
         Write_Log(Path);
		return(-3);
	}
    	 sprintf(Path,"Service - OK");
         Write_Log(Path);
	return(0);
};
