...
strcat(MDIName,".mdi");
	...
	memset(&HeaderInfo.StartZeit[0],0x00,sizeof(struct IndexHeader_t));
	 ...
	 i=read(RecorderIndexFile,&HeaderInfo.StartZeit[0],fl);
  close(RecorderIndexFile);
     ...
     i=read(RecorderFile,&Buffer[0],8);
     if ( i != 8 ) {
		   close(RecorderFile);
		   RecorderFile=-1;
		   return(FALSE);
	 ...
	 RecorderTyp=-1;
	   ...
	   _lseeki64(RecorderFile,0,SEEK_SET);
 ...
 _lseeki64(RecorderFile,-32000,SEEK_END);
 i=1;
  while ( i > 0 ) {
	 i=Load_Next_Buffer(&RecorderFile,3,&Buffer[0]);
    ...
    LONG lParam)
{
    ...
    CHAR szFilter[] = "MultiDec DVR-Formate (*.pva;*.mpg)\0*.pva;*.mpg;\0Technotrend PVA (*.pva)\0*.pva\0 MPEG-2 (*.mpg)\0*.mpg\0\0";
 ...
 OpenFileName.lpstrTitle        = "DVR-Dateien abspielen";
				...
				if ( Timer >= 10 ) {
					...
					if ( Play == TRUE ) {
        			  ...
        			  DeleteObject(hBrush);
						...
						strcpy(Text,"<Keine Datei geladen>");
                        SetDlgItemText(hDlg,TEXT2,Text);
		 ...
		 if (LOWORD(wParam) == IDJUMP ) {
				...
				if ( TimeStamp < HeaderInfo.SpielZeit ) {
					  Jump_Time_Play(TimeStamp);
					   ...
					   MessageBox(hDlg,"Zeit größer Endzeit","Multi-Decoder",MB_ICONSTOP|MB_OK);
				};
				return(TRUE);
		 }
		 ...
		 if (LOWORD(wParam) == IDC_INSERT ) {
			    ...
			    TimeStamp=m*60+s;
    			 if (Insert_Sprungmarke(TimeStamp) == FALSE ) {
						            MessageBox(hDlg,"Max. 32 Sprungmarken erlaubt","Multi-Decoder",MB_ICONSTOP|MB_OK);
									return(TRUE);
					  };
 			  Update_MDI_File();
			  InvalidateRect(hDlg,&UpdateRect,FALSE);
				...
				if ( Timer >= 10 ) {
					...
					if ( Record == TRUE ) {
					   ...
					   SetDlgItemText(hDlg,TEXT1,Text);
						...
						strcpy(Text,"<Keine Aufnahme gestartet>");
                        SetDlgItemText(hDlg,TEXT2,Text);
					    SetDlgItemText(hDlg,TEXT1,"");
...
while ((i > 0 ) && ( TimeStampStart == 0 )) {
 ...
 _lseeki64(CheckFile,-32000,SEEK_END);
i=1;
while (i > 0 ) {
	 i=Load_Next_Buffer(&CheckFile,3,&Buffer[0]);
	 s1=Get_Seconds(Buffer);
    ...
    LONG lParam)
{
    ...
    switch (message) {
        case WM_INITDIALOG:
              sprintf(StartPfad,"Folge-Datei %s-%d nirgens gefunden !!!",RecorderName,RecordDateiNummer);
        ...
        case WM_COMMAND:
		 ...
		 if (LOWORD(wParam) == IDC_BUTTON1 ) {
                                             ...
                                             OpenFileName.nMaxFileTitle     = sizeof(szFileTitle);
                                             OpenFileName.lpstrInitialDir   = (LPCTSTR)RecordBaseDir[0];
                                             OpenFileName.lpstrTitle        = "Folge-Dateien abspielen";
                                             OpenFileName.nFileOffset       = 0;
                                             OpenFileName.nFileExtension    = 0;
                                             OpenFileName.lpstrDefExt       = (LPCTSTR)"*.pva-*;*.mpg-*";
                                             OpenFileName.lCustData         = 0;
...
if ( RecordFormat==0) CurrentRecordMode=1;
else if ( RecordFormat==1) CurrentRecordMode=4;
else CurrentRecordMode=5;
...
if ( HeaderInfo.RecordFormat == 0 ) CurrentPlayMode=1;
else if ( HeaderInfo.RecordFormat == 1 ) CurrentPlayMode=4;
else CurrentPlayMode=5;
...
char Titel[64];
		 ...
		 ListCount=2;
         strcpy(&ListText[0][0],"Play");
         strcpy(&ListText[1][0],"Record");
    ...
    SetFont( 1, 1, 2, 0);
    strcpy(Zeile,"Digitaler Video Recorder");
    ...
    SetFont( 1, 1, 3, 1);
		 ...
		 for (;;) {
			 ...
			 if (( AltSelection >= 0 ) && ( AltSelection < ListCount )) {
					 ...
					 WriteText( (unsigned char)1, (unsigned short)60,  (unsigned short)(((AltSelection+1)*28)+5) ,ListText[AltSelection]);
			 };
			 ...
			 if (( Selection >= 0 ) && ( Selection < ListCount )) {
					 ...
					 WriteText( (unsigned char)1, (unsigned short)60,  (unsigned short)(((Selection+1)*28)+5),ListText[Selection]);
			 };
...
CreateOSDWindow( 2, (unsigned short)BITMAP4, (unsigned short)588, (unsigned short)32);
MoveWindowAbs(2,(unsigned short)20,400);
...
MoveWindowAbs(1,(unsigned short)20,440);
SetColorBlend(1);
yuv=RGB2YUV(0,0,0);
cl=(yuv&0xffff);
ch=((yuv>>16)&0xffff);
...
if ( i== 0 ) strcpy(Titel,"<Unbekannt>");
} else { 
	strcpy(Titel,"<Keine Datei geladen>");
};
...
WriteText(2,20,2,Titel);
				 ...
				 if ( Play == TRUE ) {
                        ...
                        DrawBlock(2,(unsigned short)(TESTBITMAP->bmiHeader.biWidth-100), 0,(unsigned short)100, 32,1);
...
MENU_LOAD_FILE:
	...
	strcpy(Zeile,"DVR - Datei auswählen");
	WriteText( 1, 140, 0,Zeile);
	strcpy(Zeile,"<4> Info  <5> Start <0> Zurück");
...
Rebuild_Liste:
    ...
    DrawBlock((unsigned char)1, (unsigned short)5, 28,490,337,0);
    SetFont( (unsigned char)1, (unsigned char)1, (unsigned short)1, (unsigned short)0);
	i=0;
	while (( i < 12 ) && ( i+Top < ListCount )) {
	 WriteText( (unsigned char)1, (unsigned short)20,  (unsigned short)(((i+1)*28)+5) ,ListText[Top+i]);
     i++;
	};
		 ...
		 for (;;) {
			 ...
			 if (( AltSelection >= 0 ) && ( AltSelection < ListCount )) {
            		 DrawBlock((unsigned char)1, (unsigned short)5, (unsigned short)(((AltSelection+1)*28)+4),(unsigned short)490,(unsigned short) 28,(unsigned short)0);
                     SetFont( (unsigned char)1, (unsigned char)1, (unsigned short)1, (unsigned short)0);
					 WriteText( (unsigned char)1, (unsigned short)20,  (unsigned short)(((AltSelection+1)*28)+5) ,ListText[AltSelection+Top]);
			 };

			 if (( Selection >= 0 ) && ( Selection < ListCount )) {
				     AltSelection=Selection;
            		 DrawBlock((unsigned char)1, (unsigned short)5, (unsigned short)(((Selection+1)*28)+5),(unsigned short)490, (unsigned short)27,(unsigned short)1);
                     SetFont( (unsigned char)1, (unsigned char)1, (unsigned short)0, (unsigned short)1);
					 WriteText( (unsigned char)1, (unsigned short)20,  (unsigned short)(((Selection+1)*28)+5),ListText[Selection+Top]);
			 };
...
INFO_MENU:
	...
	strcpy(Zeile,"DVR - Datei Infos");
	WriteText( 1, 190, 0,Zeile);
	if (( Play == FALSE ) && ( Record == FALSE )) {
	strcpy(Zeile,"<4> Auswahl  <5> Start <0> Zurück");
	WriteText( 1, 120, 372,Zeile);
	} else {
	strcpy(Zeile,"<0> Zurück");
	...
	strcpy(Zeile,"KB/Dateien");
    WriteText(1,4,84,Zeile);
	sprintf(Zeile,"%d KB in %d Dateien",HeaderInfo.KBTotal,HeaderInfo.Anzahl_Files);
    WriteText(1,130,84,Zeile);

	strcpy(Zeile,"Aufnahme ");
    WriteText(1,4,112,Zeile);
	sprintf(Zeile,"vom %s um %s Uhr ",HeaderInfo.AufnahmeDatum ,HeaderInfo.StartZeit);
	...
	if (( Play == FALSE ) && ( Record == FALSE )) {
			...
			sprintf(Zeile,"%d:%02d     < %d:%02d gespielt>",m,s,m1,s1);
	} else if ( Record == TRUE ) {
			m=(HeaderInfo.CurrentSpielZeit/60);
			s=HeaderInfo.CurrentSpielZeit-(m1*60);
			sprintf(Zeile,"%d:%02d aufgenommen",m,s);
	}
			 ...
			 if ( OSD_Key_Value == 0x00 ) {; 
				 ...
				 if ( ButtonSetTime[7] < 10 ) continue;
                   ...
                   goto MARK_MENU;
				    ...
				    cPos=0;
		            i=OSD_Edit_Ascii_Mask((unsigned char)2,TESTBITMAP->bmiHeader.biWidth-100,0,6,&cPos,OutBuffer,3,1);
                    if ( i == 13 ) {
                       i=1;
...
CreateOSDWindow( 3, (unsigned short)BITMAP4, (unsigned short)588, (unsigned short)18);
MoveWindowAbs(3,(unsigned short)20,470);
SetColorBlend(3);
...
DrawBlock(3, 0, 0,(unsigned short)587, (unsigned short)18,0);
	...
	while (( i < MaxY ) && ( i+Top < ListCount )) {
		 ...
		 for (;;) {
			 ...
			 if (( AltSelection >= 0 ) && ( AltSelection < MaxY )) {
            		 DrawBlock((unsigned char)1, (unsigned short)0, (unsigned short)(((AltSelection+2)*28)+4),(unsigned short)200,(unsigned short) 28,(unsigned short)1);
                     SetFont( (unsigned char)1, (unsigned char)1, (unsigned short)3, (unsigned short)1);
					 WriteText( (unsigned char)1, (unsigned short)60,  (unsigned short)(((AltSelection+2)*28)+5) ,ListText[AltSelection+Top]);
			 };

			 if (( Selection >= 0 ) && ( Selection < ListCount )) {
				     AltSelection=Selection;
            		 DrawBlock((unsigned char)1, (unsigned short)5, (unsigned short)(((Selection+2)*28)+5),(unsigned short)190, (unsigned short)27,(unsigned short)3);
                     SetFont( (unsigned char)1, (unsigned char)1, (unsigned short)0, (unsigned short)3);
					 WriteText( (unsigned char)1, (unsigned short)60,  (unsigned short)(((Selection+2)*28)+5),ListText[Selection+Top]);
			 };
...
SetBlend_(1,16,0, 0);
DrawBlock(1, 0, 0,(unsigned short)TESTBITMAP->bmiHeader.biWidth, (unsigned short)TESTBITMAP->bmiHeader.biHeight,0);
DrawBlock(2, 0, 0,(unsigned short)588, 31,1);
SetFont( 2, 1, 3, 1);
strcpy(Titel,"  Lade Bitmap  ");
WriteText(2,20,2,Titel);
...
if ( i== 0 ) strcpy(Titel,"<Unbekannt>");
	...
	strcpy(Titel,"< Recorder nicht gestartet>");
};
...
WriteText(2,20,0,Titel);
				 ...
				 if ( Record == TRUE ) {
						...
						s=HeaderInfo.CurrentSpielZeit-(m*60);
						sprintf(Zeile,"%3d:%02d",m,s);
                        WriteText(2,(unsigned short)488,0,Zeile);
				 }
...
while ( TrySync < 3 ) { 
len = read(*fd,&Buffer[0],8000);
for ( i =0; i < len; i++ ) {
	if ( Is_Valid_Sync(&Buffer[i]) == TRUE ) {
		_lseeki64(*fd,-8000+i,SEEK_CUR);
		return(TRUE);
	};
};
TrySync++;
...
int Irq_Record(unsigned char *Daten , int Laenge )
{
    ...
    CurrentTimeStamp=Get_Seconds(Daten);
	   if ( CurrentTimeStamp != 0 ) {
        ...
        memcpy(&Mpeg_Buffer[Mpeg_Buffer_Pos],&Daten[0],Laenge);
        Mpeg_Buffer_Pos+=Laenge;
					if ( Mpeg_Buffer_Pos+0x800 > MPEG_BUFFER_SIZE ) {
						
						if ( RecorderTyp == 0 ) {
						  if ( Write_Buffer(&Mpeg_Buffer[0], Mpeg_Buffer_Pos ) < 0 ) return(-1);
						} else if ( RecorderTyp == 1 ) {
                          if ( Write_as_PAV_File(&Mpeg_Buffer[0], Mpeg_Buffer_Pos) < 0 )  return(-1);
						}
						Mpeg_Buffer_Pos=0;
					};
...
{
int i;
unsigned int BytesWritten=0xffffffff;
if ( Laenge == 0 ) return(0);
	...
	HeaderInfo.Bytesplay+=Laenge;
        i=(int)(HeaderInfo.Bytesplay/1048576);
				...
				if ( BytesWritten != (unsigned int )Laenge )  {
					// Write im Eimer // Nächste Platte oder Ende 
					if ( Next_Record_File() == FALSE ) return(-1);
...
void Write_PS_Header( int fd )
{
	...
	unsigned char ps_hdr[32] = { 
		      0x00,0x00,0x01,0xBA,0x44,0x00,0x04,0x00,0x04,0x01,0x01,0xD5,0xFF,0xF8,0x00,0x00,
			  0x01,0xBB,0x00,0x0C,0x88,0x8B,0x81,0x05,0xE1,0xFF,0xC0,0xC0,0x80,0xE0,0xE0,0xE6 };

    i=write(fd,&ps_hdr[0],32);
};
...
int Play_Audio_Track(int Typ)
{
	...
	len=Load_Next_Buffer(&AudioFilePtr, 1 , &Next_Audio_Buffer[0]);
    if ( len < 0 ) return(-1);

	if ( RecorderFormat == 0 ) {
    CurrentTimeStamp=Get_Seconds(&Next_Audio_Buffer[0]);
	   if ( CurrentTimeStamp != 0 ) {
		   HeaderInfo.CurrentSpielZeit=LaufZeitSekunden(HeaderInfo.TimeStampStart,CurrentTimeStamp);
	   };
    };

	memcpy(Record_mem_dma.pUserAddr,&Next_Audio_Buffer[0],len);
	return(len);
}
...
int Play_Video_Track(int Typ )
{
	...
	len=Load_Next_Buffer(&VideoFilePtr, 0 , &Next_Video_Buffer[0]);
    if ( len < 0 ) return(-1);
    CurrentTimeStamp=Get_Seconds(&Next_Video_Buffer[0]);
	   if ( CurrentTimeStamp != 0 ) {
		   HeaderInfo.CurrentSpielZeit=LaufZeitSekunden(HeaderInfo.TimeStampStart,CurrentTimeStamp);
    };
	memcpy(Record_mem_dma.pUserAddr,&Next_Video_Buffer[0],len);
    return(len);
}
...
BOOL Is_Video_Sync(unsigned char *Header)
{
    ...
    sync|=(Header[2]<<8);
    sync|=Header[3];
    if ((sync&~0x1f)==0x000001e0) return(TRUE);
return(FALSE);
};
    ...
    LONG lParam)
{
  ...
  case WM_COMMAND:
	  ...
	  if ( LOWORD(wParam) == IDC_SM1+j ) {
		  ...
		  if ( Translated == TRUE ) {
			  if (( i < 0 ) ||( i > 59 )) {
 					MessageBox(hDlg,"Minuten zwischen 0 und 59 !!!","Multi-Decode",MB_ICONSTOP|MB_OK);
	  ...
	  if ( LOWORD(wParam) == IDC_DT1+j ) {
		  ...
		  if ( Translated == TRUE ) {
			  if (( i < 1 ) ||( i > 31 )) {
 					MessageBox(hDlg,"Tage zwischen 1 und 31 !!!","Multi-Decode",MB_ICONSTOP|MB_OK);
	  ...
	  if ( LOWORD(wParam) == IDC_DM1+j ) {
		  ...
		  if ( Translated == TRUE ) {
			  if (( i < 1 ) ||( i > 12 )) {
 					MessageBox(hDlg,"Monat zwischen 1 und 12 !!!","Multi-Decode",MB_ICONSTOP|MB_OK);
...
{
	unsigned int pti;
	if ( MPG_Header[8] < 5 ) return(0);
	pti  = (((MPG_Header[9] & 0x06) << 4)  | ((MPG_Header[10] & 0xFC) >> 2)) << 24; 
	pti |= (((MPG_Header[10] & 0x03) << 6) | ((MPG_Header[11] & 0xFC) >> 2)) << 16; 
	pti |= (((MPG_Header[11] & 0x02) << 6) | ((MPG_Header[12] & 0xFE) >> 1)) << 8;
	pti |= (((MPG_Header[12] & 0x01) << 7) | ((MPG_Header[13] & 0xFE) >> 1));
	return(pti/90000);
...
CreateOSDWindow( 1, 1, 360, (unsigned short)64);
MoveWindowAbs(1,200,100 );
SetColorBlend(1);
OSDSetColor(1,0, OSD_Colors[0].R, OSD_Colors[0].G, OSD_Colors[0].B ,OSD_Colors[0].Blend);
OSDSetColor(1,1, OSD_Colors[1].R, OSD_Colors[1].G, OSD_Colors[1].B, OSD_Colors[1].Blend);
...
SetFont( 1, 1, 3, 1);
if ( Play == TRUE ) {
	strcpy(Zeile,"DVR Wiedergabe ist");
    WriteText( 1, 80, 4,Zeile);
} else if ( Record == TRUE) {
	strcpy(Zeile,"DVR Aufnahme ist");
    WriteText( 1, 80, 4,Zeile);
} else {
	strcpy(Zeile,"Digitaler Video Recorder");
    WriteText( 1, 64, 4,Zeile);
};
strcpy(Zeile,"beendet");
...
Next_mpg_Read:
	...
	if ( i < 6 ) {
	 ...
	 if ( Typ == 0 ) {
		 // Video 
		 VideoBlocks++;
		 if ( Is_Video_Sync(&Daten[0]) == TRUE ) return(len);
	 };

	 if ( Typ == 1 ) {
		 // Audio 
		 AudioBlocks++;
		 if ( Is_Audio_Sync(&Daten[0]) == TRUE ) return(len);
	 };
...
int kpes_to_av_pes(unsigned char *buf, int *StartPos, int count, p2a *p,unsigned char *Next_Pes_Buffer)
{
	...
	while (c < count && p->found <4 ){
		...
		case 3:
			p->cid = 0;
			if (( buf[c] == PROG_STREAM_MAP ) ||
                ( buf[c] == PRIVATE_STREAM2 ) ||
                ( buf[c] == PROG_STREAM_DIR ) ||
                ( buf[c] == ECM_STREAM ) ||
                ( buf[c] == EMM_STREAM ) ||
                ( buf[c] == PADDING_STREAM ) ||
                ( buf[c] == DSM_CC_STREAM ) ||
                ( buf[c] == ISO13522_STREAM ) ||
                ( buf[c] == PRIVATE_STREAM1 )) {
							p->done = 1;
				} else if (( buf[c] >= AUDIO_STREAM_S ) &&
				       ( buf[c] <= VIDEO_STREAM_E )) {
							p->found++;
							p->cid = buf[c];
							c++;
				} else {
	...
	while (!p->done && (c < count && p->found > 5 && (p->mpeg == 0 ||
					  (p->mpeg == 1 && p->found < 7) ||
					  (p->mpeg == 2 && p->found < 9)))){
		if ( p->found == 6 ){
			p->flag1 = buf[c];
	...
	if ( p->done || (p->mpeg == 2 && p->found >= 9)  || 
	     (p->mpeg == 1 && p->found >= 7) ){
			...
			if (( p->cid >= VIDEO_STREAM_S ) &&
				( p->cid <= VIDEO_STREAM_E )) {
			...
			while (c < count && ((unsigned int)p->found < p->plength+6
					     || !p->plength)){
				max = MAX_AV - ((p->apsv.flags & PTS_FLAG)>>2) 
				  - p->apsv.length;
				l = count-c;
				if (p->plength && (unsigned int)l+p->found > p->plength+6)
					l = p->plength+6-p->found;
				if (l < max){
					memcpy(p->apsv.payload+p->apsv.length,
					       buf+c,l);
					p->apsv.length += l;
					p->found += l;
					c += l;
        ...
        } else if (( p->cid >= AUDIO_STREAM_S ) &&
				( p->cid <= AUDIO_STREAM_E )) {
