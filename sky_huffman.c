...
// TODO:
// The dictionaries are fixed and should not be. They should be
// updated by the data on PID 85.
...
void sky_decode(unsigned char *src, unsigned char *dst, int max_input_length, int max_decode_length)
{
	...
	while(TRUE)
	{
		...
		if(dlen >= max_decode_length)
			break;
		if(ilen >= max_input_length) 
			break;
